...
import { createCraftsmanAction, deleteCraftsmanAction, updateCraftsmanAction } from "./actions";

type Draft = Pick<Craftsman, "name" | "specialty" | "phone" | "note">;

const emptyDraft: Draft = { name: "", specialty: "", phone: "", note: "" };

export function CraftsmenTable({ initialCraftsmen }: { initialCraftsmen: Craftsman[] }) {
  const [items, setItems] = useState<Craftsman[]>(initialCraftsmen);
  const [drafts, setDrafts] = useState<Record<string, Draft>>({});
  const [newItem, setNewItem] = useState<Draft>(emptyDraft);
  const [error, setError] = useState<string | null>(null);

  function getDraft(item: Craftsman): Draft {
  ...
  async function handleSave(item: Craftsman) {
    setError(null);
    try {
      const draft = getDraft(item);
      const updated = await updateCraftsmanAction({ id: item.id, ...draft });
  ...
  async function handleDelete(id: string) {
    setError(null);
    try {
      await deleteCraftsmanAction(id);
  ...
  async function handleAdd() {
    setError(null);
    try {
      const created = await createCraftsmanAction(newItem);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 p-4 pb-24 sm:p-8">
      <header>
        <h1 className="text-xl font-bold text-neutral-900">職人管理</h1>
        <p className="mt-1 text-sm text-neutral-500">工程管理で作業を割り当てる職人・協力業者の一覧です。</p>
      </header>
          ...
          <table className="w-full min-w-[640px] text-sm">
            <thead>
              <tr className="border-b border-neutral-200 text-left text-neutral-500">
                <th className="px-4 py-3 font-medium">名前・会社名</th>
                <th className="px-4 py-3 font-medium">専門</th>
                  ...
                  <tr key={item.id} className="border-b border-neutral-100 last:border-0">
                    ...
                    <td className="px-4 py-2">
                      <input
                        ...
                        onChange={(e) => updateDraft(item.id, { specialty: e.target.value })}
                      />
                    </td>
                    <td className="px-4 py-2">
                      <input
      ...
      <section className="space-y-4 rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
        <h2 className="text-sm font-semibold text-neutral-700">新しい職人を登録</h2>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <input
            className="rounded border border-neutral-300 px-3 py-2"
            placeholder="名前・会社名（必須）"
            value={newItem.name}
            onChange={(e) => setNewItem((prev) => ({ ...prev, name: e.target.value }))}
          />
          <input
            className="rounded border border-neutral-300 px-3 py-2"
            placeholder="専門（大工・電気等）"
